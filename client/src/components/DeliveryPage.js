...
function DeliveryPage() {
  ...
  const handlePaymentChange = async (orderId, paymentReceived, paymentMethod, amountReceived) => {
    ...
    try {
      const res = await fetch('/api/delivery/orders/payment', {
        ...
        body: JSON.stringify({ orderId, paymentReceived, paymentMethod, amountReceived })
      });
  ...
  const handleAmountBlur = (order) => {
    ...
    handlePaymentChange(order.orderId, order.paymentReceived, order.paymentMethod, order.amountReceived);
  };
  ...
  const handleCheckboxToggle = (order) => {
    ...
    handlePaymentChange(order.orderId, isNowChecked, order.paymentMethod, newAmountReceived);
  };
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer bg-white border border-gray-200 rounded p-1.5 hover:border-gray-300 shadow-sm">
                        ...
                        <span className={`font-bold ${order.paymentReceived ? 'text-green-700' : 'text-gray-600'}`}>
                          {order.paymentReceived ? 'Received ✅' : 'Collect Payment'}
                        </span>
                      </label>
                        ...
                        <div className="flex items-center gap-3 bg-gray-50 px-2 py-1 rounded border border-gray-200">
                          <label className="flex items-center gap-1 cursor-pointer">
                            <input 
                              ...
                              onChange={() => handlePaymentChange(order.orderId, order.paymentReceived, 'Cash', order.amountReceived)}
                          ...
                          <label className="flex items-center gap-1 cursor-pointer">
                            <input 
                              ...
                              onChange={() => handlePaymentChange(order.orderId, order.paymentReceived, 'GPay', order.amountReceived)}
                              className="w-3.5 h-3.5 text-jts-red focus:ring-jts-red"
                            />
